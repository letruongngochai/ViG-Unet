...
def train_model(
    ...
    # (Initialize logging)
    experiment = wandb.init(project='ViG-UNet', resume='allow', anonymous='must')
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs)  # goal: maximize Dice score

    grad_scaler = torch.cuda.amp.GradScaler(enabled=amp)
    criterion = nn.CrossEntropyLoss() + 0.5 * nn.BCEWithLogitsLoss() if model.n_classes > 1 else nn.BCEWithLogitsLoss()
    global_step = 0
